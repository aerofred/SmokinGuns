...
void NET_SetMulticast6(void)
{
	...
	else
	{
		...
		for(i = 0; i < numIP; i++)
		{
			if(localIP[i].type == NA_IP6 && localIP[i].ifname[0] && Q_stricmp(localIP[i].ifname, "lo0"))
			{
				curgroup.ipv6mr_interface = if_nametoindex(localIP[i].ifname);
				if(curgroup.ipv6mr_interface)
				{
					Com_Printf("NET_SetMulticast6: using interface %s\n", localIP[i].ifname);
					break;
				}
			}
		}
...
void NET_JoinMulticast6(void)
{
	...
	{
		Com_Printf("NET_JoinMulticast6: skipped, no IPv6 socket\n");
		return;
	}

	if(multicast6_socket != INVALID_SOCKET)
	{
		Com_Printf("NET_JoinMulticast6: already joined\n");
		return;
	}

	if(net_enabled->integer & NET_DISABLEMCAST)
	{
		Com_Printf("NET_JoinMulticast6: skipped, multicast disabled (net_enabled=%i)\n", net_enabled->integer);
		return;
	}
	...
	Com_Printf("NET_JoinMulticast6: joined %s on interface %u\n", net_mcast6addr->string, curgroup.ipv6mr_interface);
}
...
#if defined(__linux__) || defined(MACOSX) || defined(__BSD__) || defined(__APPLE__)
	...
	while(1)
	{
		...
		if(NET_GetPacket(&from, &netmsg, fdr))
		{
			...
			if(com_sv_running->integer)
			{
				...
				// Connectionless packets (e.g. LAN infoResponse
				// announcements from other servers) are also useful to
				// the local client's server browser even while we host,
				// so forward them. The server ignores these anyway.
